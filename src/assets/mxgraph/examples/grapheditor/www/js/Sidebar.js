 ...
 */
Sidebar.prototype.addEmployeePalette = function (expand) {

    var fns = [];
    var arr = getdata();    
    for (var i = 0; i <= arr.length-1; i++) {
        fns.push(this.createVertexTemplateEntry('text;strokeColor=none;fillColor=none;',
               0, 0, arr[i], 'Employee',null,null, 'label'));
   ...
   // alert(fns);
    this.addemployeeFunctions('Employee', "Employee", (expand != null) ? expand : true, fns);
}

Sidebar.prototype.addModulepal = function (expand)
{
   ...
   var data1= get_model();
    var sds =data1;
    // sds = JSON.parse(sds);
    for (var i = 0; i <= sds.length - 1; i++) {
        fns.push(this.createVertexTemplateEntry('text;strokeColor=none;fillColor=none;',
             0, 0, sds[i].legoName, 'Module', null, null, 'label'));
    }
    this.addemployeeFunctions('Module', "Module", (expand != null) ? expand : true, fns);
    $("#alert_meassage").css("display", "none");
    sessionStorage.setItem("sd", null);
}
Sidebar.prototype.addemployeeFunctions = function (id, title, expanded, fns)
{
...
Sidebar.prototype.init = function()
{
	...
	this.addEmployeePalette(false);
	this.addModulepal(false);
...
Sidebar.prototype.addGeneralPalette = function(expand)
{
	...
	var fns = [
        ...
        //html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=middle;whiteSpace=wrap;
	 	this.createVertexTemplateEntry('text;',
 			40, 20, 'Text', 'Text', null, null, 'text textbox textarea label'),
            //html=1;strokeColor=none;fillColor=none;spacing=5;spacingTop=-20;whiteSpace=wrap;
	 	this.createVertexTemplateEntry('text;html=1;', 190, 120,
...
function click_ev_item() {
    var left_s = ($(".geDiagramContainer.geDiagramBackdrop").width()) * (90 / 100);
    var top_s = ($(".geDiagramContainer.geDiagramBackdrop").height()) * (90 / 100);
    $(".geDiagramContainer.geDiagramBackdrop").scrollLeft(left_s);
    $(".geDiagramContainer.geDiagramBackdrop").scrollTop(top_s);

    var left_s1 = ($(".geDiagramContainer").width()) * (90 / 100);
    var top_s1 = ($(".geDiagramContainer").height()) * (90 / 100);
    $(".geDiagramContainer").scrollLeft(left_s1);
    $(".geDiagramContainer").scrollTop(top_s1);
}
/**
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	var elt = document.createElement('a');
	elt.setAttribute('href', 'javascript:void(0);');
	elt.setAttribute('onclick', 'click_ev_item()');
...
Sidebar.prototype.createDragSource = function(elt, dropHandler, preview, cells, bounds)
{
	...
	dragSource.stopDrag = function()
	{
	    ...
	    sessionStorage.setItem("isgraphchanged", 1);
	    //alert('TEST-1');
	    var left_s1 = ($(".geDiagramContainer").width()) * (90 / 100);
	    var top_s1 = ($(".geDiagramContainer").height()) * (90 / 100);
	    $(".geDiagramContainer").scrollLeft(left_s1);
