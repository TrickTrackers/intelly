...
mxPrintPreview.prototype.open_for_im = function (css, targetWindow, forcePageBreaks, keepOpen) {
    //debugger;
    // Closing the window while the page is being rendered may cause an
    // exception in IE. This and any other exceptions are simply ignored.
    var previousInitializeOverlay = this.graph.cellRenderer.initializeOverlay;
    ...
    try {
        ...
        for (var i = 0; i < vpages; i++) {
            ...
            for (var j = 0; j < hpages; j++) {
                ...
                var pageNum = i * hpages + j + 1;
                var clip = new mxRectangle(dx, dy, availableWidth, availableHeight);
                div = this.renderPage_for_im(this.pageFormat.width, this.pageFormat.height, 0, 0, mxUtils.bind(this, function (div) {
                    this.add_grap_frag(-dx, -dy, this.scale, pageNum, div, clip);
...
mxPrintPreview.prototype.renderPage_for_im = function (w, h, dx, dy, content, pageNumber) {
    //	var doc = this.wnd.document;
    var div = document.createElement('div');
    var arg = null;
...
mxPrintPreview.prototype.add_grap_frag = function (dx, dy, scale, pageNumber, div, clip) {
    //debugger;
    var view = this.graph.getView();
    var previousContainer = this.graph.container;
    this.graph.container = div;
