...
import * as XLSX from 'xlsx';
// import * as XLSXStyle from 'xlsx-style';
import { ExcelConfig } from "../../services/excel.config";
...
export class ExcelService {
  ...
  public exportAsExcelFile(json: any[], fileName: string): void {
    // this.worksheet = XLSX.utils.json_to_sheet(json);//.json_to_sheet(json, {header: [], skipHeader:true});
    //add styles
    this.worksheet = XLSX.utils.json_to_sheet(json);
    this.worksheet.A1.v = "Task Name"; this.worksheet.B1.v = "Card Name";
    this.worksheet.C1.v = "Start Date"; this.worksheet.D1.v = "End Date";
    this.worksheet.E1.v = "Responsible Party"; this.worksheet.F1.v = "Priority";
    this.worksheet.G1.v = "% Complete"; this.worksheet.H1.v = "Labels";
    this.worksheet.I1.v = "Board Name"; this.worksheet.J1.v = "Status";
    ...
    switch (fileName) {
      case 'TaskListReport': {
        workbook = { Sheets: { 'Task_List': this.worksheet }, SheetNames: ['Task_List'] };
        break;
      }
  ...
  private setStyles(fileName: string) {
    ...
    switch (fileName) {
      case 'TaskListReport': {
        ...
        wscols = [{ wch: 20 },{ wch: 20 }, { wch: 10 }, { wch: 10 }, { wch: 30 }, { wch: 10 },
        { wch: 10 }, { wch: 30 }, { wch: 10 }, { wch: 10 }, { wch: 10 }];
        ...
        for (let R = range.s.r + 1; R <= range.e.r; ++R) {
          ...
          const tskname = StyleUtils.encode_cell({ c: 0, r: R });
          const name = StyleUtils.encode_cell({ c: 1, r: R });
          const sdate = StyleUtils.encode_cell({ c: 2, r: R });
          const edate = StyleUtils.encode_cell({ c: 3, r: R });
          const resp = StyleUtils.encode_cell({ c: 4, r: R });
          const prior = StyleUtils.encode_cell({ c: 5, r: R });
          const comp = StyleUtils.encode_cell({ c: 6, r: R });
          const lbl = StyleUtils.encode_cell({ c: 7, r: R });
          this.setTextFmt(tskname);this.setTextFmt(name);
          this.setTextFmt(sdate); this.setTextFmt(edate);
          this.setTextFmt(resp); this.setTextFmt(prior);
          this.setTextFmt(comp); this.setTextFmt(lbl);
