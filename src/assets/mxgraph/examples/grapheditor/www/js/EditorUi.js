...
EditorUi.prototype.init = function () {
	...
	// setTimeout(() => {
	// 	if (setFileName()) {
	// 		var forceDialog = true, saveType = "save", isSave = true;
	// 		this.customSaveFile(forceDialog, saveType, isSave);
	// 	}
	// }, 10);
...
var file_name1 = "";
var update_ch = false;
EditorUi.prototype.saveFile = function (forceDialog, file1) {
	// debugger; 
	if (!forceDialog && this.editor.filename != null) {
		...
		if (this.editor.filename == null || file1 == "saveas") {
			update_ch = false;
			this.showDialog(dlg.container, 300, 100, true, true);
			dlg.init();
		} else {
			update_ch = true;
			this.save(this.editor.filename);
...
/*
	=> customs save file option
*/
EditorUi.prototype.customSaveFile = function (forceDialog, saveType, isSave) {
	if (isSave) {
		this.saveFile(forceDialog, saveType);
	}
	else {
...
EditorUi.prototype.save = function (name) {
	...
	if (name != null) {
		...
		var chk_is_v = "true";
		if (update_ch == false) {
			$.when(checkdupName(name), setTimeout(function () {
				chk_is_v = sessionStorage.getItem("check_is_v");
			}, 3000)
			).then(function () {
				setTimeout(function () {
					if (chk_is_v == "true") {
						...
						var graph1 = this.editor.graph;
						var preview = new mxPrintPreview(graph1, 1);
						var img_data = preview.open_for_im();
						img_data = "<!DOCTYPE html><html><head><title></title></head><body>" + img_data + "</body></html>";
						upload_save(xml.outerHTML, name, update_ch, img_data);
						sessionStorage.setItem("isgraphchanged", 0);
					} else {
						alert("The file name already exists.  Please use a un-used name");
...
EditorUi.prototype.save = function (name) {
	...
	if (name != null) {
		...
		if (chk_is_v == "true") {
			...
			$.iaoAlert({
				msg: "The file name already exists.  Please use an unused name",
				type: "error",
				mode: "dark"
			}
			)
